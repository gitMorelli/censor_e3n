...
def main():
    ...
    if os.path.exists(csv_modified_path)==False: #if the csv has not been preprocessed yet
        df = preprocess_df(csv_load_path,'object_name', 'e3n_id_hand')
        df.to_csv(csv_modified_path)
    load_preprocessed_df(csv_modified_path) #load the preprocessed df
 
    #load
    #ignore lines with used=TRUE
    #Enable to select a subset of ids if needed (eg if you want to consider the PD patients before the others); report which were already used
    # iterate on the ids and consider the filenames corresponding to that id


    pdf_paths = list_files_with_extension(save_path, ["pdf",'tif'], recursive=False)
...
def load_preprocessed_df(file_path):
