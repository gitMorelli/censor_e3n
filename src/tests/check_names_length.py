...
def main():
    ...
    output_dir = os.path.join(args.output_path,'name_properties')
    pdf_path = args.pdf_path
    os.makedirs(output_dir, exist_ok=True) # Create folder if it doesn't exist

    for n_questionnaire in range(1,14):
        report_file_path = os.path.join(output_dir, f"report_length_Q{n_questionnaire}.txt")

        print(f"Processing Q{n_questionnaire}...") # Keep a small progress indicator in terminal

        questionnaire_folder_path = pdf_path+f"\\Q{n_questionnaire}"
        #questionnaire_file_list = list_files_with_extension(questionnaire_folder_path,extension = ['tiff','pdf'])
        #questionnaire_file_names = [get_basename(f, remove_extension=True) for f in questionnaire_file_list]
        with os.scandir(questionnaire_folder_path) as entries:
            questionnaire_file_names = [e.name for e in entries if e.is_file()]

        #get the length of each filename in the folder and print the distribution of lengths
        filename_lengths = [len(name) for name in questionnaire_file_names]
        length_distribution = pd.Series(filename_lengths).value_counts().sort_index()
        underscore_counts = [name.count('_') for name in questionnaire_file_names]
        underscore_distribution = pd.Series(underscore_counts).value_counts().sort_index()
        with open(report_file_path, 'w') as f:
            # 1. Distribution of filename lengths
            f.write("Distribution of filename lengths in the folder:\n")
            f.write(length_distribution.to_string() + "\n\n")

            # 2. 20 shortest filenames (Fixed: removed .to_string() and used join)
            f.write("20 shortest filenames in the folder:\n")
            shortest_20 = sorted(questionnaire_file_names, key=len)[:20]
            f.write("\n".join(shortest_20) + "\n\n")

            # 3. Distribution of underscore counts
            f.write("Distribution of underscore counts in the folder:\n")
            f.write(underscore_distribution.to_string() + "\n\n")

            # 4. 20 filenames with least underscores (Fixed: removed .to_string() and used join)
            f.write("20 filenames with the least underscores in the folder:\n")
            least_underscores_20 = sorted(questionnaire_file_names, key=lambda x: x.count('_'))[:20]
            f.write("\n".join(least_underscores_20) + "\n\n")
