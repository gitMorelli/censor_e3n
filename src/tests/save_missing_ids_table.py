...
from src.utils.file_utils import get_basename, create_folder
...
def main():
    ...
    if os.path.exists(covariates_file_path):
        ...
        #initialize the df
        results = pd.DataFrame(table_ids, columns=['ident_projet'])

        with open(report_file_path, 'w') as f:
            f.write(f"table has {len(table_ids)} unique ids\n")
        for q in questionnairres:
            #initialize empty column for current quesitonnairre
            results[f'q{q}_missing'] = 0
            ...
            ids_with_q_filled = set(occurrencies[occurrencies[f'rempli_q{q}'] == 1]['ident_projet'].to_list())
            filled_but_not_in_metadata = ids_with_q_filled - metadata_ids
            ...
            with open(report_file_path, 'a') as f:
                ...
                f.write(f"Ids that should be filled but are not in metadata: {len(filled_but_not_in_metadata)}\n")
                f.write(f"Examples {list(filled_but_not_in_metadata)[:min(3,len(filled_but_not_in_metadata))]}\n")
            ...
            results.loc[results['ident_projet'].isin(filled_but_not_in_metadata), f'q{q}_missing'] = 1 
            #save the individual list of names
            missing_ids_df = pd.DataFrame(list(filled_but_not_in_metadata), columns=['ident_projet'])
            individual_file_save_path = os.path.join(output_dir, 'individual_files',f"missing_ids_q{q}.csv")
            create_folder(os.path.dirname(individual_file_save_path), parents=True, exist_ok=True)
            missing_ids_df.to_csv(individual_file_save_path, index=False)
        # save results
        results.to_csv(os.path.join(output_dir, "missing_ids_by_questionnaire.csv"), index=False)
